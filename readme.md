...
浏览器和web应用服务器之间的交互过程，遵守Http协议。没有HTTP协议就没有web，所以作为Web开发者，需要把Http协议作为底层基础来学习。另外在排查远程接口调用、缓存、资源加载或者前端安全性问题，都离不开对Http报文的分析。

（下面关于Http协议内容（Http/2除外）的图例来自《图解Http》一书）
...
接下来就是Http报文体的基本格式和详细内容。

![](img/7.png)

从上图可以看到，一个Http报文包括首部和报文主体，二者之间是空行。每个Http请求都是一个请求报文发出去，然后等待服务端的响应报文，一来一回，构成了Http的基本生命周期。在此基础上，我们需要去探究Http是如何实现长连接的，各种不同的资源类型是如何传输的，尤其是大文件的上传和下载的原理。

并不是每个发送出去的Http请求都会按照预想的结果得到响应，为了便捷的判断服务端响应状态，Http设计了响应状态码。

![](img/8.png)

状态码总共有60几种，常用的也就10几种，需要熟悉。

理解了Http协议之后，考虑到Http协议在传输安全上的缺陷，出现了TLS、SSL和Https。

![](img/9.png)

这些概念延伸到了密码学领域，但是你只需要了解并表达清楚基本交互原理就可以了。

在Html5被正式推出的时候，全双工的Websocket协议正式成为标准，这给前端开发带来了一把实时通讯和长连接的利剑。

![](img/10.png)

关于Websocket你需要了解其握手机制，和Http协议的区别，以及标准的应用场景。

千呼万唤，经历一段长久的沉寂，Http/2正式成为标准，我们需要重点了解以下内容：
1. 二进制分帧层。HTTP/2 所有性能增强的核心在于新的二进制分帧层，它定义了如何封装 HTTP 消息并在客户端与服务器之间传输。


![](img/11.png)

2. 请求与响应复用。在 HTTP/1.x 中，如果客户端要想发起多个并行请求以提升性能，则必须使用多个 TCP 连接（请参阅使用多个 TCP 连接）。 这是 HTTP/1.x 交付模型的直接结果，该模型可以保证每个连接每次只交付一个响应（响应排队）。 更糟糕的是，这种模型也会导致队首阻塞，从而造成底层 TCP 连接的效率低下。HTTP/2 中新的二进制分帧层突破了这些限制，实现了完整的请求和响应复用：客户端和服务器可以将 HTTP 消息分解为互不依赖的帧，然后交错发送，最后再在另一端把它们重新组装起来。

![](img/12.png)

3. 数据流优先级。HTTP/2 标准允许每个数据流都有一个关联的权重和依赖关系，可以向每个数据流分配一个介于 1 至 256 之间的整数。每个数据流与其他数据流之间可以存在显式依赖关系。

![](img/13.png)

4. 每个来源一个连接。有了新的分帧机制后，HTTP/2 不再依赖多个 TCP 连接去并行复用数据流；每个数据流都拆分成很多帧，而这些帧可以交错，还可以分别设定优先级。 因此，所有 HTTP/2 连接都是永久的，而且仅需要每个来源一个连接，随之带来诸多性能优势。

5. 流控制。流控制是一种阻止发送方向接收方发送大量数据的机制，以免超出后者的需求或处理能力：发送方可能非常繁忙、处于较高的负载之下，也可能仅仅希望为特定数据流分配固定量的资源。
6. 服务器推送。服务器可以对一个客户端请求发送多个响应。 换句话说，除了对最初请求的响应外，服务器还可以向客户端推送额外资源，而无需客户端明确地请求。
 
![](img/14.png)

7. 标头压缩。为了减少此开销和提升性能，HTTP/2 使用 HPACK 压缩格式压缩请求和响应标头元数据。


### 1.1.3 JavaScript 

我们目前讨论JavaScript主要包括四个组成部分，ECMAScript，文档对象模型、浏览器对象模型和Node.js。

ECMAScript是语言规范，JavaScript编程基础来源于此。很多同学在面试的过程中会被问到词法、句法这些被认为偏门的问题，还有执行上下文等被认为难度较高的问题，其实这些都是语言规范的一部分，是确确实实的语言基础。你每天都在使用，但是却不知道这些内容在规范中是如何定义的。

这里推荐大家完整阅读ES6的规范
http://www.ecma-international.org/ecma-262/6.0/index.html。

规范文档并不像书籍那么容易阅读，大家可以先阅读ES6的入门书籍，然后回头再看规范文档。比如：
